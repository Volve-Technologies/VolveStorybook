    ...
    <div className={cn('relative', sizeClasses[size], className)}>
      <div className="absolute h-full w-full">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="bg-primary-10 absolute animate-[spinner-fade_1.2s_linear_infinite] rounded-full"
            style={{
              top: '0',
              left: '50%',
...
export function PulseDotLoader({
  ...
  return (
    <div
      className={cn(
        'bg-primary-10 animate-[pulse-dot_1.2s_ease-in-out_infinite] rounded-full',
        sizeClasses[size],
        className,
      )}
    >
    ...
    <div className={cn('flex items-center space-x-1', containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={cn(
            'bg-primary-10 animate-[bounce-dots_1.4s_ease-in-out_infinite] rounded-full',
            dotSizes[size],
          )}
          style={{
  ...
  return (
    <div className={cn('flex items-center space-x-1', containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={cn('bg-primary-10 animate-[typing_1s_infinite] rounded-full', dotSizes[size])}
          style={{
    ...
    <div className={cn('flex items-center gap-0.5', containerSizes[size], className)}>
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
          className={cn(
            'bg-primary-10 animate-[wave_1s_ease-in-out_infinite] rounded-full',
            barWidths[size],
          )}
          style={{
    ...
    <div className={cn('flex', containerSizes[size], className)}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={cn(
            'bg-primary-10 h-full animate-[wave-bars_1.2s_ease-in-out_infinite]',
            barWidths[size],
          )}
          style={{
...
export function TerminalLoader({
  ...
  return (
    <div className={cn('flex items-center space-x-1', containerSizes[size], className)}>
      ...
      <div
        className={cn('bg-primary-10 animate-[blink_1s_step-end_infinite]', cursorSizes[size])}
      />
      <span className="sr-only">Loading</span>
    </div>
  );
